...
.controller('FutureSkillsController', ['$scope','profiledata', 'Account', function($scope, profiledata, Account){
	...
	$scope.add = function(){
		profiledata.futureSkills({github: $scope.profile, python: $scope.python, java: $scope.java, swift: $scope.swift, go: $scope.go, ruby: $scope.ruby, angular: $scope.angular, node: $scope.node, jQuery: $scope.jQuery, html: $scope.html, css: $scope.css})
	}
